...
summary_table <- function(active_usage, lab_notebooks) {
  ...
  usage_table <- merge(usage_table, notebook_table, by.x = "Unit", by.y = "Unit")
  
  usage_table <- usage_table[, c("Unit", "Labs", "Notebooks", "Individuals", "Activities Last 30 Days", "Data Stored (GB)")]%>%
